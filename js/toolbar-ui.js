...
class ToolbarUI {
    /**
     * Constructs a new ToolbarUI instance.
     *
     * @constructor
     */
    constructor() {
    ...
    /**
     * Updates the stop button color and visibility after a run starts.
     * @param {string} color - CSS color value for the stop icon.
     */
    highlightStop(color) {
    ...
    /**
     * Temporarily dims the stop button then restores its color (used when restarting a run).
     * @param {string} color - Final color to restore.
     */
    dimThenRestoreStop(color) {
        const stopBtn = document.getElementById("stop");
        if (!stopBtn) return;
        if (this._dimTimeout) {
            clearTimeout(this._dimTimeout);
